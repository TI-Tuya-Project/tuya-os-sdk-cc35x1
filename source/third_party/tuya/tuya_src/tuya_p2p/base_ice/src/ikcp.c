...
int ikcp_recv2(ikcpcb *kcp, char *buf, int len)
{
    assert(kcp);
    IKCPSEG *seg_ret = NULL;
    int ret = 0;

    if (iqueue_is_empty(&kcp->rcv_queue))
        return 0;

    // IKCP_RECVQUEUE_LOCK(kcp);
    ...
    if (len >= seg_ret->len) {
        ...
        // ikcp_log(kcp, IKCP_LOG_OUTPUT, "kcp recv kcp wait_rcv_bytes %d\n", kcp->wait_rcv_bytes);
        // IKCP_RECVQUEUE_UNLOCK(kcp);
        ...
        // ikcp_log(kcp, IKCP_LOG_OUTPUT, "kcp recv kcp2 wait_rcv_bytes %d\n", kcp->wait_rcv_bytes);
        // IKCP_RECVQUEUE_UNLOCK(kcp);
    }
...
// parse data
//---------------------------------------------------------------------
void ikcp_parse_data(ikcpcb *kcp, IKCPSEG *newseg, const char *data, int len)
{
        ...
        if (cmd == IKCP_CMD_ACK) {
            ...
            if (_itimediff(sn, kcp->rcv_nxt + kcp->rcv_wnd) < 0) {
                ...
                if (_itimediff(sn, kcp->rcv_nxt) >= 0) {
                    ...
                    ikcp_parse_data(kcp, seg, data, len);
                }
            }
...
// read cmd
IUINT8 ikcp_getcmd(const void *ptr)
{
    IUINT8 cmd;
    // jump conv
    ptr += 4;
    ikcp_decode8u((const char *)ptr, &cmd);
    return cmd;
}

// read conv
IUINT32 ikcp_getsn(const void *ptr)
{
    IUINT32 sn;
    // jump conv
    ptr += 4;
    ...
    // jump ts
    ptr += 4;
    ikcp_decode32u((const char *)ptr, &sn);
    return sn;
}

void ikcp_setprocesspkt(ikcpcb *kcp, int (*process_pkt)(void *user, int length, const char *input, char *output))
{
    kcp->process_pkt = process_pkt;
    return;
}
