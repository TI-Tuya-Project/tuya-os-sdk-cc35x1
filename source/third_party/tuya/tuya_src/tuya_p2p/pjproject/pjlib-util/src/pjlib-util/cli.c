 ...
 */
static pj_status_t get_available_cmds(pj_cli_sess *sess, pj_cli_cmd_spec *cmd, pj_str_t *cmd_val, unsigned argc,
                                      pj_pool_t *pool, pj_bool_t get_cmd, pj_cli_parse_mode parse_mode,
                                      pj_cli_cmd_spec **p_cmd, pj_cli_exec_info *info);
...
PJ_DEF(void) pj_cli_write_log(pj_cli_t *cli, int level, const char *buffer, int len)
{
...
PJ_DEF(void) pj_cli_sess_write_msg(pj_cli_sess *sess, const char *buffer, pj_size_t len)
{
    struct pj_cli_front_end *fe;
...
static pj_status_t cmd_handler(pj_cli_cmd_val *cval)
{
    ...
    case CLI_CMD_CHANGE_LOG:
        ...
        if (!level && cval->argv[1].slen > 0 && (cval->argv[1].ptr[0] < '0' || cval->argv[1].ptr[0] > '9')) {
...
PJ_DEF(pj_status_t) pj_cli_create(pj_cli_cfg *cfg, pj_cli_t **p_cli)
{
    ...
    pool = pj_pool_create(cfg->pf, "cli", PJ_CLI_POOL_SIZE, PJ_CLI_POOL_INC, NULL);
    if (!pool)
        return PJ_ENOMEM;
    ...
    for (i = 0; i < sizeof(cmd_xmls) / sizeof(cmd_xmls[0]); i++) {
        ...
        if (pj_cli_add_cmd_from_xml(cli, NULL, &xml, &cmd_handler, NULL, NULL) != PJ_SUCCESS) {
...
static pj_cli_cmd_spec *get_cmd_name(const pj_cli_t *cli, const pj_cli_cmd_spec *group, const pj_str_t *cmd)
{
    pj_str_t cmd_val;
...
static void add_cmd_name(pj_cli_t *cli, pj_cli_cmd_spec *group, pj_cli_cmd_spec *cmd, pj_str_t *cmd_name)
{
    pj_str_t cmd_val;
    pj_str_t add_cmd;
    char cmd_ptr[MAX_CMD_HASH_NAME_LENGTH];
    ...
    pj_hash_set(cli->pool, cli->cmd_name_hash, cmd_val.ptr, (unsigned)cmd_val.slen, 0, cmd);
}
 ...
 **/
static pj_status_t add_choice_node(pj_cli_t *cli, pj_xml_node *xml_node, pj_cli_arg_spec *arg,
                                   pj_cli_get_dyn_choice get_choice)
{
    ...
    if (arg->stat_choice_cnt > 0) {
        ...
        arg->stat_choice_val =
            (pj_cli_arg_choice_val *)pj_pool_zalloc(cli->pool, arg->stat_choice_cnt * sizeof(pj_cli_arg_choice_val));
 ...
 **/
static pj_status_t add_arg_node(pj_cli_t *cli, pj_xml_node *xml_node, pj_cli_cmd_spec *cmd, pj_cli_arg_spec *arg,
                                pj_cli_get_dyn_choice get_choice)
{
    pj_xml_attr *attr;
    pj_status_t status = PJ_SUCCESS;
 ...
 **/
static pj_status_t add_cmd_node(pj_cli_t *cli, pj_cli_cmd_spec *group, pj_xml_node *xml_node,
                                pj_cli_cmd_handler handler, pj_cli_cmd_spec **p_cmd, pj_cli_get_dyn_choice get_choice)
{
    ...
    while (attr != &root->attr_head) {
        if (!pj_stricmp2(&attr->name, "name")) {
            ...
            if (attr->value.slen) {
                ...
                if (!pj_hash_get(cli->cmd_id_hash, &cmd_id, sizeof(pj_cli_cmd_id), NULL))
                    is_valid = PJ_TRUE;
            }
            if (!is_valid)
             ...
             */
            pj_scan_init(&scanner, attr->value.ptr, attr->value.slen, PJ_SCAN_AUTOSKIP_WS, &on_syntax_error);
    ...
    while (sub_node != (pj_xml_node *)&root->node_head) {
        if (!pj_stricmp2(&sub_node->name, "CMD")) {
            status = add_cmd_node(cli, cmd, sub_node, handler, NULL, get_choice);
            if (status != PJ_SUCCESS)
                return status;
        } else if (!pj_stricmp2(&sub_node->name, "ARG")) {
            /* Get argument attribute */
            status = add_arg_node(cli, sub_node, cmd, &args[cmd->arg_cnt], get_choice);
    ...
    if (cmd->arg_cnt) {
        ...
        cmd->arg = (pj_cli_arg_spec *)pj_pool_zalloc(cli->pool, cmd->arg_cnt * sizeof(pj_cli_arg_spec));
    ...
    if (cmd->sc_cnt) {
        ...
        cmd->sc = (pj_str_t *)pj_pool_zalloc(cli->pool, cmd->sc_cnt * sizeof(pj_str_t));
    ...
    pj_hash_set(cli->pool, cli->cmd_id_hash, &cmd->id, sizeof(pj_cli_cmd_id), 0, cmd);
...
PJ_DEF(pj_status_t)
pj_cli_add_cmd_from_xml(pj_cli_t *cli, pj_cli_cmd_spec *group, const pj_str_t *xml, pj_cli_cmd_handler handler,
                        pj_cli_cmd_spec **p_cmd, pj_cli_get_dyn_choice get_choice)
{
    pj_pool_t *pool;
    pj_xml_node *root;
    pj_status_t status = PJ_SUCCESS;
...
pj_cli_sess_parse(pj_cli_sess *sess, char *cmdline, pj_cli_cmd_val *val, pj_pool_t *pool, pj_cli_exec_info *info)
{
    ...
    if (len > 0) {
        pj_scan_init(&scanner, cmdline, len, PJ_SCAN_AUTOSKIP_WS, &on_syntax_error);
        PJ_TRY
        {
            val->argc = 0;
            while (!pj_scan_is_eof(&scanner)) {
                info->err_pos = (int)(scanner.curptr - scanner.begin);
                if (*scanner.curptr == '\'' || *scanner.curptr == '"' || *scanner.curptr == '{') {
                ...
                status = get_available_cmds(sess, cmd, &str, val->argc - 1, pool, PJ_TRUE, parse_mode, &next_cmd, info);
    ...
    if ((parse_mode == PARSE_NEXT_AVAIL) || (parse_mode == PARSE_EXEC)) {
        ...
        status = get_available_cmds(sess, cmd, NULL, val->argc, pool, (parse_mode == PARSE_NEXT_AVAIL), parse_mode,
                                    NULL, info);
...
PJ_DECL(pj_status_t) pj_cli_sess_exec(pj_cli_sess *sess, char *cmdline, pj_pool_t *pool, pj_cli_exec_info *info)
{
    pj_cli_cmd_val val;
    pj_status_t status;
...
static pj_bool_t hint_inserted(const pj_str_t *name, const pj_str_t *desc, const pj_str_t *type, pj_cli_exec_info *info)
{
    unsigned i;
    for (i = 0; i < info->hint_cnt; ++i) {
        pj_cli_hint_info *hint = &info->hint[i];
        if ((!pj_strncmp(&hint->name, name, hint->name.slen)) && (!pj_strncmp(&hint->desc, desc, hint->desc.slen)) &&
...
/** This will insert new hint with the option to check for the same
    previous entry **/
static pj_status_t insert_new_hint2(pj_pool_t *pool, pj_bool_t unique_insert, const pj_str_t *name,
                                    const pj_str_t *desc, const pj_str_t *type, pj_cli_exec_info *info)
{
...
static pj_status_t insert_new_hint(pj_pool_t *pool, const pj_str_t *name, const pj_str_t *desc, const pj_str_t *type,
...
static pj_status_t get_comp_match_cmds(const pj_cli_t *cli, const pj_cli_cmd_spec *group, const pj_str_t *cmd_val,
                                       pj_pool_t *pool, pj_cli_cmd_spec **p_cmd, pj_cli_exec_info *info)
{
    pj_cli_cmd_spec *cmd;
...
static pj_status_t get_pattern_match_shortcut(const pj_cli_t *cli, const pj_str_t *cmd_val, pj_pool_t *pool,
                                              pj_cli_cmd_spec **p_cmd, pj_cli_exec_info *info)
{
    ...
    while (it) {
        unsigned i;
        pj_cli_cmd_spec *cmd = (pj_cli_cmd_spec *)pj_hash_this(cli->cmd_name_hash, it);
        ...
        for (i = 0; i < cmd->sc_cnt; ++i) {
            ...
            if (!pj_strncmp(sc, cmd_val, cmd_val->slen)) {
                ...
                status = insert_new_hint2(pool, PJ_TRUE, sc, &cmd->desc, &SHORTCUT, info);
                if (status != PJ_SUCCESS)
                    return status;
...
static pj_status_t get_pattern_match_cmds(pj_cli_cmd_spec *cmd, const pj_str_t *cmd_val, pj_pool_t *pool,
    ...
    if (cmd->sub_cmd) {
        ...
        while (child_cmd != cmd->sub_cmd) {
            ...
            if (!pj_strncmp(&child_cmd->name, cmd_val, cmd_val->slen)) {
                status = insert_new_hint(pool, &child_cmd->name, &child_cmd->desc, NULL, info);
                if (status != PJ_SUCCESS)
                    return status;
            ...
            if (found) {
                if (parse_mode == PARSE_NEXT_AVAIL) {
                    ...
                    for (i = 0; i < child_cmd->sc_cnt; ++i) {
                        ...
                        status = insert_new_hint(pool, sc, &child_cmd->desc, &SHORTCUT, info);
                        if (status != PJ_SUCCESS)
                            return status;
                    }
                }
...
static pj_status_t get_match_args(pj_cli_sess *sess, pj_cli_cmd_spec *cmd, const pj_str_t *cmd_val, unsigned argc,
                                  pj_pool_t *pool, pj_cli_parse_mode parse_mode, pj_cli_exec_info *info)
{
    pj_cli_arg_spec *arg;
    pj_status_t status = PJ_SUCCESS;
        ...
        if (arg->type == PJ_CLI_ARG_CHOICE) {
            ...
            for (j = 0; j < arg->stat_choice_cnt; ++j) {
                ...
                if (!pj_strncmp(&choice_val->value, cmd_val, cmd_val->slen)) {
                    status = insert_new_hint(pool, &choice_val->value, &choice_val->desc,
            ...
            if (cmd_val->slen == 0) {
                if (info->hint_cnt == 0) {
                    if (!((parse_mode == PARSE_EXEC) && (arg->optional))) {
                        ...
                        status = insert_new_hint(pool, &arg->name, &arg->desc, &arg_type[arg->type].msg, info);
                        if (status != PJ_SUCCESS)
                            return status;
                    }
                    if (!arg->optional)
                                      ...
                                      pj_cli_cmd_spec **p_cmd, pj_cli_exec_info *info)
{
    ...
    if (get_cmd) {
        status = get_comp_match_cmds(sess->fe->cli, cmd, prefix, pool, p_cmd, info);
        if (status != PJ_SUCCESS)
            return status;

        /** If exact match found, then no need to search for pattern match **/
        if (info->hint_cnt == 0) {
            if ((parse_mode != PARSE_NEXT_AVAIL) && (cmd == &sess->fe->cli->root)) {
                /** Pattern match for shortcut needed on root command only **/
                status = get_pattern_match_shortcut(sess->fe->cli, prefix, pool, p_cmd, info);

                if (status != PJ_SUCCESS)
                    return status;
            }

            status = get_pattern_match_cmds(cmd, prefix, pool, p_cmd, parse_mode, info);
        }
